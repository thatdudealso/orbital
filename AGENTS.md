...
- The levels/worlds are code-authoritative in `src/orbital/game/levels.ts` (campaign order + per-level segments/tips) and `src/orbital/game/biomes.ts` (name, gravity, tagline/briefing, palette). `src/orbital/ui/level-select.tsx` just renders those two. If the README's "worlds" table and this code ever disagree, the code wins - update the README, not the other way around.
- No `LICENSE` file exists in this repo; the intended open-source license hasn't been decided. Don't add one without an explicit decision from the maintainer.
